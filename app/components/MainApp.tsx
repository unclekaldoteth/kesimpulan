...
import { 
  ...
  CheckCircle, XCircle, Wallet, Clipboard, Loader2, Heart, Image as ImageIcon
} from 'lucide-react';
...
export default function Home() {
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      link.href = dataUrl;
      link.click();
      showToast("NFT berhasil disimpan! 🖼️");
...
const exportImage = async (): Promise<string | null> => {
  ...
  const handleTip = async (amountEth: string) => {
    ...
    try {
      ...
      const ethVal = amountEth === '1' ? '0.0003' : '0.0015'; // $1 & $5 estimasi
      ...
      <div className="px-4 pt-6 max-w-md mx-auto space-y-6 relative z-10">
        ...
        {activeTab === 'quiz' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            {!quizData ? (
                // STATE: BELUM ADA RINGKASAN (INPUT)
                <>
                ...
                // STATE: SUDAH ADA RINGKASAN (RESULT)
                <div className="space-y-6 animate-in slide-in-from-bottom-4">
                    
                    {/* VISUAL MAP CARD (Tanpa Tombol Save, Bersih) */}
                    ...
                    {/* QUIZ SECTION (Dengan Tombol Retry) */}
                    ...
                    {isCorrect && (
                        <div className="flex flex-col gap-3 animate-pulse">
                            ...
                            <button 
                                onClick={handleMint} 
                                disabled={isMinting}
                                className={`w-full py-4 rounded-xl font-bold text-white border transition-all flex justify-center items-center gap-2 ${isMinting ? 'bg-[#222] border-white/5 cursor-wait' : 'bg-pink-600 border-pink-500/50 hover:bg-pink-500 shadow-[0_0_20px_rgba(219,39,119,0.3)]'}`}
                            >
        ...
        {/* TAB 2: LEADERBOARD (Dummy Tampilan Keren) */}
        ...
        {activeTab === 'profile' && (
            <div className="space-y-6 animate-in fade-in">
                ...
                <div className="bg-[#111] border border-white/10 rounded-2xl p-5">
                    <div className="flex items-center gap-2 mb-4"><Heart size={16} className="text-red-500" fill="currentColor"/><h3 className="text-sm font-bold text-white">Support Creator</h3></div>
                    <div className="flex flex-col gap-3">
                        <button onClick={() => sdk.actions.openUrl("https://warpcast.com/unclekal")} className="w-full py-3 bg-[#1a1a1a] hover:bg-[#222] border border-white/5 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95 group">
                            <User size={18} className="text-blue-400 group-hover:scale-110 transition-transform"/><span className="text-sm font-bold text-white">Follow @unclekal</span>
                        </button>
                        <div className="grid grid-cols-2 gap-3">
                            <button onClick={() => handleTip('1')} className="bg-[#1a1a1a] hover:bg-[#222] border border-white/5 rounded-xl p-4 flex flex-col items-center gap-2 transition-all active:scale-95">
                                <Wallet size={20} className="text-blue-400"/><span className="text-xs font-bold text-white">Tip ~ $1</span>
                            </button>
                            <button onClick={() => handleTip('5')} className="bg-[#1a1a1a] hover:bg-[#222] border border-white/5 rounded-xl p-4 flex flex-col items-center gap-2 transition-all active:scale-95">
                                <Sparkles size={20} className="text-yellow-400"/><span className="text-xs font-bold text-white">Tip ~ $5</span>
                            </button>
                        </div>
                    </div>
                </div>
